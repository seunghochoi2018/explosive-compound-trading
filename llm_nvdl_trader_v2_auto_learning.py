    ...
    def _evaluate_strategies(self, recent_trades: List[Dict]) -> List[Dict]:
        ...
        for i, insight in enumerate(sorted_insights):
            ...
            if improvement > 0:
                ...
                effective_strategies.append(insight)
                print(f"[⚠️ KEEP] 전략 #{i+1} 유지 (적용 전: {win_rate_before:.1f}% → 이후: {win_rate_after:.1f}%, {improvement:+.1f}%)")
            else:
...
📊 현재 성과:
- 최근 {len(recent_trades)}거래 승률: {win_rate:.1f}% ⚠️ 너무 낮음!
...
⚠️ 반드시 위 전략과 "완전히 다른" 새로운 관점의 전략을 만들어야 합니다!
⚠️ 예제를 복사하지 말고, 실제 데이터에서 새로운 패턴을 발견하세요!

⚠️⚠️⚠️ 절대 금지 사항 - 하드코딩된 고정값 사용 금지! ⚠️⚠️⚠️
...
⚠️ 중요: 위 예제를 복사하지 말고, 실제 데이터에서 새로운 패턴을 찾으세요!
⚠️ 5개 전략은 모두 서로 다른 관점이어야 하며, 기존 전략과도 달라야 합니다!
