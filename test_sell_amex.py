...
SOXL 1주 매도 테스트 - AMEX + SOXL 티커 사용
"""
import json
import yaml
import requests
...
print("="*80)
print("SOXL 1주 매도 테스트 - AMEX + SOXL 티커")
print("="*80)
...
# 테스트 1: AMEX + SOXL 티커
print(f"\n{'='*80}")
print(f"[테스트 1] AMEX + SOXL 티커")
print(f"{'='*80}")

sell_data = {
    "CANO": account_no.split('-')[0],
    "ACNT_PRDT_CD": account_no.split('-')[1],
    "OVRS_EXCG_CD": "AMEX",  # ⚠️ AMEX 사용
    "PDNO": "SOXL",  # ⚠️ 티커 그대로
    "ORD_QTY": "1",
...
response = requests.post(order_url, headers=sell_headers, json=sell_data, timeout=30)
...
if response.status_code == 200:
    ...
    if result.get('rt_cd') == '0':
        ...
        # 테스트 2: NASD + SOXL 티커
        print(f"\n{'='*80}")
        print(f"[테스트 2] NASD + SOXL 티커")
        print(f"{'='*80}")

        sell_data["OVRS_EXCG_CD"] = "NASD"
        print(json.dumps(sell_data, indent=2, ensure_ascii=False))

        hashkey = generate_hashkey(sell_data)
        sell_headers["hashkey"] = hashkey
