...
# 텔레그램 및 Ollama 헬퍼
from telegram_helpers import TelegramNotifier, start_ollama
...
class KISLLMTrader:
    ...
    def __init__(self, config_path: str = None):
        ...
        print(f"  [INFO] 텔레그램 알림 모드: {'활성화' if self.enable_telegram else '비활성화'}")
        print(f"  [INFO] 실제 거래: 비활성화 (알림만 전송)")
        print("="*80)
    ...
    def send_trading_signal_notification(self, signal: str, symbol: str, confidence: float, reasoning: str, current_position: str = None, current_pnl_pct: float = 0):
        ...
        if self.enable_telegram and self.telegram:
            action = 'BUY' if signal == 'BULL' else 'SELL'
            ...
            print(f"[텔레그램] {signal} 신호 알림 전송 완료")
        else:
            print("[INFO] 텔레그램 비활성화 상태")
    ...
    def place_order(self, symbol: str, order_type: str, quantity: int = None, price: float = None) -> Dict:
        ...
        """
        print(f"\n[텔레그램 알림] {order_type} {symbol} {quantity or '최대'}주 (실제 주문 안 함)")
...
"""
    ...
    def execute_llm_strategy(self):
        ...
        # 1.5. 트레일링 스탑 체크 (텔레그램 모드에서는 건너뜀)
        if False and current_position:
            print(f"\n[리스크 관리 - 트레일링 스탑]")
        ...
        # 5. 포지션 전환 감지 (텔레그램 모드: 알림만)
        if should_switch:
            print(f"\n[포지션 전환 권장]")
            print(f"  현재: {current_position} (손익 {current_pnl_pct:+.2f}%)")
            print(f"  권장: {target_symbol} ({signal})")
            ...
            self.send_trading_signal_notification(
                ...
                confidence=confidence,
                reasoning=f"포지션 전환 권장: {current_position} → {target_symbol}\n{llm_analysis['reasoning']}",
                current_position=current_position,
                current_pnl_pct=current_pnl_pct
            )

            # 텔레그램 모드: 거래 기록 저장 생략
            print(f"  [INFO] 포지션 전환 알림 완료 (거래 기록 저장 생략)")

        # 6. 신호가 바뀔 때만 텔레그램 알림
        if signal != self.current_llm_direction and target_symbol:
            print(f"\n[신호 변화 감지!]")
