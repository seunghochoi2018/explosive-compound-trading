...
class NVDLNVDQSignalNotifier:
    def __init__(self, fmp_api_key: str):
        ...
        self.learning_config = {
            'min_results_for_learning': 10,    # 최소 학습 데이터 수
            'learning_frequency': 24,          # 24시간마다 재학습
            'success_threshold': 0.6,          # 성공 임계값
            'confidence_adjustment_factor': 0.1, # 신뢰도 조정 비율
        }
    ...
    def format_signal_message(self, signal: TradingSignal) -> str:
        ...
        if signal.action == "BUY":
            ...
            message_parts.extend([
                ...
                f"[TIME] **예상보유**: {signal.holding_period}",
        ...
        message_parts.extend([
            f"[TIME] **발신시간**: {signal.timestamp.strftime('%H:%M:%S')}",
            "",
 ...
 **모니터링 대상**: NVDL, NVDQ
[TIME] **체크 간격**: {self.config['check_interval']//60}분
...
[TIME] **시작**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
...
[TIME] **보유시간**: {result.holding_duration:.1f}시간
