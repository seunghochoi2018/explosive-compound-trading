...
class KISLLMTrader:
    ...
    def __init__(self, config_path: str = None):
        ...
        # [REMOVED] 고정 손절선 제거 - LLM이 시장 상황 보고 스스로 판단
        ...
        # - 고정 임계치는 특정 상황에서만 유효 (레버리지 ETF는 노이즈 크지만, 그것도 상황마다 다름)
        # - LLM이 과거 성공/실패 패턴 학습 → 상황별 자율 판단
        # - 예: 급등장 -3% vs 횡보장 -3% → 의미 완전히 다름
        # self.trailing_stop_loss = -3.0     # 제거됨
        # self.max_pnl = -999.0              # 제거됨
    ...
    def get_usd_cash_balance(self, symbol: str = "TQQQ", price: float = 35.0, exchange_cd: str = None) -> Dict:
        """
        USD 현금 잔고 조회
        ...
        # 거래소 코드 처리 (파라미터 우선, 없으면 매핑에서 가져오기)
...
"""
            ...
            if analysis and analysis.get('final_decision'):
                ...
                # final_decision (BUY/SELL/HOLD/CLOSE) → signal (BULL/BEAR/HOLD) 변환
                decision_to_signal = {
                    'BUY': 'BULL',
                    'SELL': 'BEAR',
                    'HOLD': 'HOLD',
                    'CLOSE': 'HOLD'  # CLOSE = 포지션 유지 (SOXL/SOXS는 항상 둘 중 하나 보유)
                }
    ...
    def execute_llm_strategy(self):
        ...
        if current_position:
            ...
            # [DISABLED] 트레일링 스탑 로직 비활성화 - LLM 자율 판단으로 전환
            # 이유: 시장 상황마다 적절한 손절선이 다름. LLM이 과거 패턴 학습해서 판단
            #
            # # 최고 PNL 갱신
            ...
            # [NEW] LLM 자율 판단 모드: 포지션 정보만 표시
            print(f"  현재 손익: {current_pnl_pct:+.2f}%")
            print(f"  전략: LLM이 시장 맥락 보고 청산/유지 자율 판단 (고정 손절선 없음)")
        ...
        if current_position:
            ...
            if signal != self.current_llm_direction:
                ...
                # 피라미딩 체크 (고도화 버전 - 복리 극대화)
                if not hasattr(self, 'pyramid_levels'):
                    self.pyramid_levels = 0

                # 단계별 피라미딩 전략
                # 1단계: PNL +3%, 신뢰도 80%, +30% 증액
                # 2단계: PNL +8%, 신뢰도 85%, +25% 증액
                # 3단계: PNL +15%, 신뢰도 90%, +20% 증액

                pyramid_executed = False

                if self.pyramid_levels == 0 and current_pnl_pct >= 3.0 and confidence >= 80:
                    pyramid_qty = int(positions[0]['qty'] * 0.30)  # 30% 증액
                    if pyramid_qty > 0:
                        print(f"\n[PYRAMID-1] 조건 충족: PNL {current_pnl_pct:+.2f}% ≥ 3%, 신뢰도 {confidence}% ≥ 80%")
                        print(f"[PYRAMID-1] 증액: +30% ({pyramid_qty}주)")
                        buy_result = self.buy_stock(current_position, pyramid_qty)
                        if buy_result['success']:
                            self.pyramid_levels = 1
                            pyramid_executed = True
                            print(f"[PYRAMID-1] 1단계 완료")

                elif self.pyramid_levels == 1 and current_pnl_pct >= 8.0 and confidence >= 85:
                    pyramid_qty = int(positions[0]['qty'] * 0.25)  # 25% 증액
                    if pyramid_qty > 0:
                        print(f"\n[PYRAMID-2] 조건 충족: PNL {current_pnl_pct:+.2f}% ≥ 8%, 신뢰도 {confidence}% ≥ 85%")
                        print(f"[PYRAMID-2] 증액: +25% ({pyramid_qty}주)")
                        buy_result = self.buy_stock(current_position, pyramid_qty)
                        if buy_result['success']:
                            self.pyramid_levels = 2
                            pyramid_executed = True
                            print(f"[PYRAMID-2] 2단계 완료")

                elif self.pyramid_levels == 2 and current_pnl_pct >= 15.0 and confidence >= 90:
                    pyramid_qty = int(positions[0]['qty'] * 0.20)  # 20% 증액
                    if pyramid_qty > 0:
                        print(f"\n[PYRAMID-3] 조건 충족: PNL {current_pnl_pct:+.2f}% ≥ 15%, 신뢰도 {confidence}% ≥ 90%")
                        print(f"[PYRAMID-3] 증액: +20% ({pyramid_qty}주)")
                        buy_result = self.buy_stock(current_position, pyramid_qty)
                        if buy_result['success']:
                            self.pyramid_levels = 3
                            pyramid_executed = True
                            print(f"[PYRAMID-3] 3단계 완료")

                if not pyramid_executed:
                    if self.pyramid_levels >= 3:
                        print(f"  전략: 최대 피라미딩 도달 (3/3) - 보유 유지")
                    else:
                        next_pnl = [3.0, 8.0, 15.0][self.pyramid_levels]
                        next_conf = [80, 85, 90][self.pyramid_levels]
                        print(f"  전략: 추세가 계속되므로 보유 (레벨 {self.pyramid_levels}/3)")
                        print(f"  다음 피라미딩: PNL ≥ {next_pnl}%, 신뢰도 ≥ {next_conf}%")
                else:
                    print(f"  [PYRAMID] {self.pyramid_levels}단계 완료 (총 레버리지: ~{1 + self.pyramid_levels * 0.25:.2f}x)")

                # [중요] 레버리지 ETF 감가상각 방지: SOXL/SOXS 둘 다 시간 제한
                time_limit_reached = False  # 시간 도달 플래그
                if self.entry_time:
                    holding_seconds = (datetime.now() - self.entry_time).total_seconds()
                    holding_minutes = holding_seconds / 60

                    if holding_seconds >= self.max_position_time:
                        print(f"\n[⏰ 최대 보유 시간 도달]")
                        print(f"  종목: {current_position}")
                        print(f"  보유 시간: {holding_minutes:.1f}분 (최대: {self.max_position_time/60:.0f}분)")
                        print(f"  이유: 3배 레버리지 ETF 감가상각(decay) 방지")
                        print(f"  현재 손익: {current_pnl_pct:+.2f}%")
                        print(f"  → 강제 청산 후 재진입 대기")
                        should_switch = True  # 강제 청산 트리거
                        time_limit_reached = True  # 시간 도달 표시
                    else:
                        remaining_minutes = (self.max_position_time - holding_seconds) / 60
                        print(f"  보유 시간: {holding_minutes:.1f}분 (잔여: {remaining_minutes:.1f}분)")
        ...
        # 6. 신규 진입 (포지션 전환 포함)
        # [중요] 시간 도달로 청산한 경우 재진입하지 않음 (감가상각 방지)
        if 'time_limit_reached' not in locals():
            time_limit_reached = False

        if time_limit_reached:
            print(f"\n[대기 모드] 시간 도달 청산 후 재진입 대기 중 (다음 사이클에 재평가)")

        if (should_switch or not current_position) and target_symbol and usd_cash > 0 and not time_limit_reached:
            print(f"\n[매수 예정] {target_symbol}")
            ...
            # 피라미딩 레벨 초기화 (새 포지션 시작)
            self.pyramid_levels = 0
            print(f"  [피라미딩 초기화] 레벨 0/3")
    ...
    def run(self, interval_seconds: int = 300):
        ...
        # ============================================================
        # [REMOVED] Ollama 자동 재시작 로직 제거 - 사용자 요청
        # ============================================================
        # 주석: "ollama 없으면 실행시키지마"
        # 주석: "이건 주기적으로 재시작하는거 아냐? ollama"
        #
        # 기존 로직:
        #   - 4시간마다 자동으로 Ollama 프로세스 종료 후 재시작
        #   - GPU 메모리 정리 목적
        #
        # 문제:
        #   - 여러 트레이더가 동시에 재시작 시도 → 프로세스 중복
        #   - 포트 충돌 (11434, 11435 동시 점유)
        #   - 재시작 타이밍에 타임아웃 발생
        #
        # 새 정책:
        #   - Ollama는 사용자가 수동으로 관리
        #   - 트레이더는 Ollama가 실행 중일 때만 동작
        #   - GPU 메모리 관리는 Ollama 자체 기능 사용
        # ============================================================
...
def main():
    ...
    # 자동매매 무한 루프 (백테스트 기반: 1시간마다 LLM 분석)
    # 백테스트 결과: 1-2일 보유 시 승률 66.7%
    trader.run(interval_seconds=3600)  # 1시간 (3600초)
